...
@section('content')
                ...
                <form class="flex-1">
                    <div class="relative w-full">
                        <input type="search" id="search-dropdown" name="search" value="{{ request('search') }}"
                            ...
                            placeholder="إبحث عن فئة بالإسم" />
                        @if (request('search'))
                            <a href="{{ route('categories.index', array_merge(request()->except('search'), request()->only(['warehouse_id', 'category_id']))) }}"
                ...
                <div class="relative p-4 w-full max-w-md max-h-full">
                    <div class="relative bg-white rounded-lg shadow-sm">
                        ...
                        <form action="{{ route('categories.store') }}" method="POST" class="p-4 md:p-5">
                            ...
                            <div class="space-y-4">
                                <div>
                                    <label class="block text-sm font-bold text-dark mb-2">إسم الصنف بالعربي</label>
                                ...
                                <div>
                                    <label class="block text-sm font-bold text-dark mb-2">إسم الصنف بالإنجليزي</label>
                                    <input type="text" name="name_en" required
                                        class="w-full p-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent transition-all duration-200">
                                </div>
                                <div>
                                    <label class="block text-sm font-bold text-dark mb-2">إسم الصنف الأب</label>
                                    <select id="parentSelect" name="parent_id"
                                        ...
                                        <option value="">اختر الصنف الأب (اختياري)</option>
                ...
                <table class="w-full border-collapse bg-white rounded-lg overflow-hidden shadow-sm">
                    <thead>
                        <tr class="bg-gray-200">
                            ...
                            <th class="p-4 text-center font-bold text-gray-700 border-b border-gray-200">الفئة الأب</th>
                    ...
                    <tbody>
                        @forelse ($categories as $category)
                                    ...
                                    <div class="relative bg-white rounded-lg shadow-sm">
                                        ...
                                        <div class="p-4">
                                            <form action="{{ route('categories.update', $category) }}" method="POST" class="pt-3">
                                                ...
                                                <div class="space-y-4">
                                                    <div>
                                                        <label class="block text-sm font-bold text-dark mb-2">إسم الفئة بالعربي</label>
                                                        <input type="text" id="editName" name="name_ar" required value="{{ $category->name_ar }}"
                                                    ...
                                                    <div>
                                                        <label class="block text-sm font-bold text-dark mb-2">إسم بالانجليزي</label>
                                                        <input type="text" id="editName" name="name_en" required value="{{ $category->name_en }}"
                                                            class="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent transition-all duration-200">
                                                    </div>
                                                    <div>
                                                        <label class="block text-sm font-bold text-dark mb-2">إسم الصنف الأب</label>
                                                        <select id="parentSelectEdit{{ $category->id }}" name="parent_id"
                                                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent transition-all duration-200 block w-full p-1">
                                                            <option value="">اختر الصنف الأب (اختياري)</option>
                                                            @foreach ($categories as $cat)
                                                                    <option value="{{ $cat->id }}" {{ ($category->parent->id ?? '') == $cat->id ? 'selected' : '' }}>
                                                                        {{ $cat->name_ar }}
                                                                    </option>
                                                            @endforeach
                                                        </select>
                            ...
                            {{-- delete modal --}}
                            <div id="delete-modal{{ $category->id }}" tabindex="-1" class="modal hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                                <div class="relative p-4 w-full max-w-md max-h-full">
                                    <div class="relative bg-white rounded-lg shadow-sm">
                                        ...
                                        <div class="p-4 md:p-5 text-center">
                                            ...
                                            <h3 class="mb-5 text-lg font-normal text-dark">هل انت متأكد من حذف الفئة <strong>{{ $category->name_ar }}</strong>؟</h3>
                                            ...
                                            <form action="{{ route('categories.destroy', $category) }}" method="POST">
                                                @csrf
                                                @method('DELETE')
                                                <button data-modal-hide="delete-modal{{ $category->id }}" type="submit"
                            ...
                            <tr>
                                <td colspan="5" class="p-8 text-center text-gray-500">
                                    <div class="flex flex-col items-center">
                                        ...
                                        <p class="text-lg">لا توجد فئات متاحة</p>
                                        <p class="text-sm text-gray-400 mt-2">ابدأ بإضافة فئة جديدة</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            // Initialize edit modal select2 for each category
            @foreach ($categories as $cat)
                $('#parentSelectEdit{{ $cat->id }}').select2({
                    placeholder: 'اختر الصنف الأب (اختياري)',
                    allowClear: true,
                    dir: 'rtl',
                    language: {
