...
@section('content')
        ...
        <div id="purchaseRequestsContainer" class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            @forelse ($purchaseRequests as $request)
                <div class="purchase-request-card overflow-hidden bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300"
                    ...
                    data-search="{{ $request->id }} {{ $request->warehouse->name ?? '' }}"
                        ...
                        <div class="flex justify-between items-start mb-4">
                            <div>
                                ...
                                <p class="text-sm text-gray-600 mt-1">
                                    ...
                                    {{ $request->warehouse->name_ar ?? 'غير محدد' }}
                                </p>
                            </div>
                            <span
                                class="status-badge px-3 py-1 rounded-full text-xs font-medium
                                    {{ $request->status === 'مكتمل' ? 'bg-green-100 text-green-800' : '' }}
                                    {{ $request->status === 'ملغي' ? 'bg-red-100 text-red-800' : '' }}
                                    {{ $request->status === 'في الانتظار' ? 'bg-yellow-200 text-yellow-800' : '' }}">
                                {{ $request->status }}
                            </span>
                        </div>
                        {{-- @if ($request->notes)
                                    ...
                                    @foreach ($request->items as $item)
                                        <div class="flex justify-between items-center bg-gray-100 p-2 rounded-lg">
                                            <span class="text-sm text-gray-700 font-medium">
                                                <i class="fas fa-circle text-orange-500 ml-2"
                                                    style="font-size: 0.375rem;"></i>
                                                {{ $item->product->name_ar }}
                                            </span>
                                            <span
                                                class="text-sm font-bold text-orange-600 bg-orange-100 px-2 py-1 rounded-xl">{{ $item->quantity ?? 0 }}</span>
                                        </div>
                                    @endforeach
                                </div>
                            @endif
                    ...
                    <div class="p-6 bg-white">
                        <div class="flex flex-wrap gap-2">
                            ...
                            <button onclick="toggleOffers({{ $request->id }})"
                                ...
                                <i
                                    class="fas fa-chevron-down ml-1 expand-icon-{{ $request->id }} transition-transform duration-300"></i>
                                عرض العروض (<span class="offers-count">{{ count($request->offers ?? []) }}</span>)
                            </button>
                            @if ($request->purchaseOrder)
                                <div
                                    class="w-full bg-green-100 text-green-700 text-sm px-3 py-2 rounded-lg cursor-default flex items-center justify-center">
                                    <i class="fas fa-check-circle ml-2"></i>
                                    تم إنشاء أمر شراء لهذا الطلب
                                </div>
                            @elseif(count($request->offers ?? []) > 0)
                            ...
                            @if (count($request->offers ?? []) > 0)
                                <div class="space-y-3">
                                    @foreach ($request->offers as $offer)
                                        <div class="offer-card bg-white p-4 rounded-lg border-2 border-gray-200 hover:border-gray-300 cursor-pointer transition-all duration-200 relative {{ $offer->selected ? 'selected' : '' }}"
                                            onclick="selectOffer({{ $offer->id }}, {{ $request->id }})"
                                            data-offer-id="{{ $offer->id }}" data-request-id="{{ $request->id }}">
                                            <!-- Checkbox -->
                                            <div
                                                class="offer-checkbox absolute top-2 left-2 w-4 h-4 border-2 border-gray-300 rounded bg-white {{ $offer->selected ? '' : 'hidden' }}">
                                                <i
                                                    class="fas fa-check text-white text-xs absolute inset-0 flex items-center justify-center"></i>
                                            </div>

                                            <div class="flex justify-between items-start">
                                                <div class="flex-1">
                                                    <h5 class="font-medium text-gray-800">
                                                        {{ $offer->supplier->name_ar ?? 'مورد غير محدد' }}</h5>
                                                </div>
                                                <div class="text-left">
                                                    <p class="text-lg font-bold text-orange-600">
                                                        {{ number_format($offer->total_price, 2) }} ريال</p>
                                                </div>
                                            </div>
                                            @if ($offer->notes)
    ...
    <div id="addPurchaseRequestModal" tabindex="-1" aria-hidden="true"
            ...
            <div class="relative bg-white rounded-lg shadow-xl">
                ...
                <form action="{{ route('purchase-requests.store') }}" method="POST" class="p-4 md:p-5">
                    ...
                    <div class="flex items-center space-x-3">
                        <button type="submit"
                            class="text-white bg-primary hover:opacity-90 focus:ring-4 focus:outline-none focus:ring-orange-300 font-bold rounded-lg text-sm px-5 py-2.5 text-center flex items-center">
                            حفظ الطلب
                        </button>
                        <button type="button" onclick="closeAddPurchaseRequestModal()"
    ...
    <div id="addOfferModal" tabindex="-1" aria-hidden="true"
        class="modal hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
        <div class="fixed inset-0 bg-gray-900/80 transition-opacity duration-300"></div>
        <div class="relative p-4 w-full max-w-md max-h-full z-60">
            <div class="relative bg-white rounded-lg shadow-xl">
                ...
                <form id="addOfferForm" action="{{ route('purchase-offers.store') }}" method="POST"
                    class="p-4 md:p-5">
                    @csrf
                    ...
                    <div class="grid gap-4 mb-4">
                        ...
                        <div>
                            <label for="total_price" class="block mb-2 text-sm font-medium text-gray-900">السعر
                                الإجمالي</label>
                        ...
                        <div>
                            ...
                            <textarea name="notes" id="offer_notes" rows="3"
                    ...
                    <div class="flex items-center">
                        <button type="submit"
                            class="w-full text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-bold rounded-lg text-sm px-5 py-2.5 text-center flex items-center justify-center">
                            حفظ العرض
                        </button>
                        <button type="button" onclick="closeAddOfferModal()"
                            class="w-full py-2.5 px-5 text-sm font-bold text-gray-900 focus:outline-none bg-gray-200 rounded-lg border border-gray-200 hover:bg-gray-100 mr-3">
                            إلغاء
                        </button>
                    </div>
                </form>
    ...
    <div id="createOrderModal" tabindex="-1" aria-hidden="true"
        ...
        <div class="relative p-4 w-full max-w-md max-h-full z-60">
            <div class="relative bg-white rounded-lg shadow-xl">
                ...
                <!-- Modal body -->
                <form id="createOrderForm" action="{{ route('purchase-orders.store') }}" method="POST"
                    class="p-4 md:p-5">
                    @csrf
                    ...
                    <div class="mb-4">
                        ...
                        <div>
                            <label for="order_notes" class="block mb-2 text-sm font-medium text-gray-900">ملاحظات
                                إضافية</label>
                            <textarea name="notes" id="order_notes" rows="3"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-2 focus:ring-orange-500 focus:outline-transparent focus:border-transparent block w-full p-2.5"
                                placeholder="أدخل أي ملاحظات إضافية لأمر الشراء..."></textarea>
                        </div>
                    </div>
                    ...
                    <div class="flex gap-3">
                        <button type="submit"
                            class="w-full text-white bg-orange-500 hover:bg-orange-600 focus:ring-4 focus:outline-none focus:ring-orange-300 font-bold rounded-lg text-sm px-5 py-2.5 text-center flex items-center justify-center">
                            <i class="fas fa-check ml-2"></i>
                            إنشاء الأمر
                        </button>
                        <button type="button" onclick="closeCreateOrderModal()"
                            class="w-full py-2.5 px-5 text-sm font-bold text-gray-900 focus:outline-none bg-gray-200 rounded-lg border border-gray-200 hover:bg-gray-100 text-center">
                            إلغاء
                        </button>
                    </div>
                </form>
